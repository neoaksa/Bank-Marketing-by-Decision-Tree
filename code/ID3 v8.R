...
pairs(~y+month+contact+pdays, data=bank)
pairs(~y+day+campaign+previous, data=bank)

# delete unrelated attribute
MyData = subset(bank,select = -c(month,contact,duration,campaign,pdays,day,previous))
#MyData = subset(bank,select = -c(duration))
